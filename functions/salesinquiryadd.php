...
foreach($_FILES['file']['name'] as $key=>$val){        
	...
	if(move_uploaded_file($_FILES['file']['tmp_name'][$key],$upload_file)){
		...
		$insert_sql = "INSERT INTO tbl_sales_inquiry(sales_inquiry_id, customer_id, product_id,material_name,product_image,product_manufacturing_process,salesinquiry_status,product_added_by,product_added_date,assignedto) 
			VALUES('','".$customer."', '".$product."','".$materialdata."','".$images_name."','".$mfgprocess."','Added', '".$username."', '".$dateToday."','Unassigned')";
		if(mysqli_query($con, $insert_sql) or die("database error: ". mysqli_error($con)))
		{
			...
			VALUES('','Sales Inquiry', 'Approval','Sales Associate','Unassigned','".$dateToday."','".$last_id."','Not yet started')";
